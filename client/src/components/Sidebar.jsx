...
function Sidebar({ sidebarOpen, setSidebarOpen }) {
    ...
    return (
        <div
            className={`w-60 xl:w-72 bg-white border-r border-gray-200 p-4 flex flex-col gap-6 absolute sm:relative z-50 h-screen transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "max-sm:-translate-x-full"}`}
        >
            <div className="w-full">
                <img onClick={() => navigate("/")} src={assets.logo} alt="logo" className="w-26 ml-7 my-2 cursor-pointer" />
                <hr className=" border-r-gray-300 mb-8"/>
